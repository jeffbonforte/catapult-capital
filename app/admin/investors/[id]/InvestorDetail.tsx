...
export default function InvestorDetail({ investor, deals }: { investor: any, deals: any[] }) {
  ...
  const [editForm, setEditForm] = useState({ name: investor.name, email: investor.email, company: investor.company || '' })
  ...
  const [isLocked, setIsLocked] = useState(investor.isLocked)
  const [confirmDelete, setConfirmDelete] = useState(false)

  async function saveProfile(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true)
    await fetch(`/api/admin/investors/${investor.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...editForm, isLocked })
    })
    setSaving(false)
    setEditing(false)
  ...
  async function toggleLock() {
    ...
    await fetch(`/api/admin/investors/${investor.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: investor.name, email: investor.email, company: investor.company, isLocked: next })
    })
    setIsLocked(next)
  ...
  async function deleteUser() {
    await fetch(`/api/admin/investors/${investor.id}`, { method: 'DELETE' })
            ...
            <div>
              <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:8}}>
                ...
                <div>
                  ...
                  {investor.company && <div style={{fontSize:13,color:'var(--slate-500)',marginTop:2}}>{investor.company}</div>}
                </div>
              </div>
              <div style={{fontSize:13,color:'var(--slate-500)'}}>
                Last login: {investor.lastLoginAt ? new Date(investor.lastLoginAt).toLocaleString() : 'Never'} · Member since {new Date(investor.createdAt).toLocaleDateString()}
              </div>
            </div>
